...
const EditChoirMinistry = () => {
  ...
  // Fetch existing Choir Ministry sections
  useEffect(() => {
    const fetchChoirMinistrySections = async () => {
      try {
        setLoading(true);
        
        const response = await fetch('http://localhost:5001/api/choir-ministry/admin', {
          headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to fetch Choir Ministry sections: ${response.status} ${errorText}`);
        }
        ...
        if (Array.isArray(data)) {
          ...
          setChoirMinistrySections(validSections);
        } else {
          console.error('Expected array but got:', typeof data, data);
          setChoirMinistrySections([]);
        }
        setError(null);
      } catch (err) {
        setError(`Failed to load Choir Ministry content: ${err.message}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  const updateSection = async (sectionId, updatedData) => {
    try {
      ...
      const response = await fetch(`http://localhost:5001/api/choir-ministry/${sectionId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`
        },
  ...
  const deleteSection = async (sectionId) => {
    ...
    try {
      ...
      setChoirMinistrySections(prev => prev.filter(section => section._id !== sectionId));
  ...
  const addNewSection = async () => {
    try {
      const newOrder = Math.max(...choirMinistrySections.map(s => s.order), 0) + 1;
      ...
      const response = await fetch('http://localhost:5001/api/choir-ministry', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        },
      ...
      setChoirMinistrySections(prev => [...prev, newSection]);
  ...
  if (authLoading) {
    return (
      <div className="edit-choir-ministry-page">
    ...
    return (
      <div className="edit-choir-ministry-page">
        <div className="error-container">
          <h2>Access Denied</h2>
          <p>You don't have permission to edit Choir Ministry content.</p>
          <button onClick={() => navigate('/choir-ministry')} className="btn-retry">
            Back to Choir Ministry
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="edit-choir-ministry-page">
      ...
      <div className="edit-header">
        <h1>Edit Choir Ministry</h1>
        <div className="header-actions">
          <button onClick={() => navigate('/choir-ministry')} className="btn-secondary">
            Back to Choir Ministry
          </button>
      ...
      <div className="sections-container">
        {choirMinistrySections && choirMinistrySections.length > 0 ? (
          choirMinistrySections
