...
const EditHarvestCommittee = () => {
  ...
  useEffect(() => {
    const fetchHarvestCommitteeSections = async () => {
      try {
        ...
        const response = await fetch('http://localhost:5001/api/harvest-committee/admin', {
          headers: {
        ...
        if (Array.isArray(data)) {
          ...
          setHarvestCommitteeSections(validSections);
        } else {
  ...
  const updateSection = async (sectionId, updatedData) => {
    try {
      ...
      const response = await fetch(`http://localhost:5001/api/harvest-committee/${sectionId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`
        },
  ...
  const addNewSection = async () => {
    try {
      const newOrder = Math.max(...harvestCommitteeSections.map(s => s.order), 0) + 1;
      ...
      const response = await fetch('http://localhost:5001/api/harvest-committee', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        },
  ...
  if (authLoading) {
    return (
      <div className="edit-harvest-committee-page">
    ...
    return (
      <div className="edit-harvest-committee-page">
        <div className="error-container">
          <h2>Access Denied</h2>
          <p>You don't have permission to edit Harvest Committee content.</p>
          <button onClick={() => navigate('/harvest-committee')} className="btn-retry">
            Back to Harvest Committee
          </button>
        </div>
      </div>
    );
  ...
  if (loading) {
    return (
      <div className="edit-harvest-committee-page">
        <div className="loading-container">
          <div className="loading-spinner"></div>
          <p>Loading Harvest Committee sections...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="edit-harvest-committee-page">
      ...
      <div className="edit-header">
        <h1>Edit Harvest Committee</h1>
        <div className="header-actions">
          <button onClick={() => navigate('/harvest-committee')} className="btn-secondary">
            Back to Harvest Committee
          </button>
