...
const EditBibleStudy = () => {
  ...
  // Fetch existing Bible Study sections
  useEffect(() => {
    const fetchBibleStudySections = async () => {
      try {
        setLoading(true);
        
        const response = await fetch('http://localhost:5001/api/bible-study/admin', {
          headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to fetch Bible Study sections: ${response.status} ${errorText}`);
        }
        ...
        if (Array.isArray(data)) {
          ...
          setBibleStudySections(validSections);
        } else {
          console.error('Expected array but got:', typeof data, data);
          setBibleStudySections([]);
        }
        setError(null);
      } catch (err) {
        setError(`Failed to load Bible Study content: ${err.message}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  const updateSection = async (sectionId, updatedData) => {
    try {
      ...
      const response = await fetch(`http://localhost:5001/api/bible-study/${sectionId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`
        },
  ...
  const deleteSection = async (sectionId) => {
    ...
    try {
      ...
      setBibleStudySections(prev => prev.filter(section => section._id !== sectionId));
  ...
  const addNewSection = async () => {
    try {
      const newOrder = Math.max(...bibleStudySections.map(s => s.order), 0) + 1;
      ...
      setBibleStudySections(prev => [...prev, newSection]);
  ...
  if (authLoading) {
    return (
      <div className="edit-bible-study-page">
    ...
    return (
      <div className="edit-bible-study-page">
        <div className="error-container">
          <h2>Access Denied</h2>
          <p>You don't have permission to edit Bible Study content.</p>
          <button onClick={() => navigate('/bible-study')} className="btn-retry">
            Back to Bible Study
          </button>
        </div>
      </div>
  ...
  return (
    <div className="edit-bible-study-page">
      ...
      <div className="edit-header">
        <h1>Edit Bible Study</h1>
        <div className="header-actions">
          <button onClick={() => navigate('/bible-study')} className="btn-secondary">
            Back to Bible Study
          </button>
      ...
      <div className="sections-container">
        {bibleStudySections && bibleStudySections.length > 0 ? (
          bibleStudySections
