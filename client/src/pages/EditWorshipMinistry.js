...
const EditWorshipMinistry = () => {
  ...
  useEffect(() => {
    const fetchWorshipMinistrySections = async () => {
      try {
        ...
        const response = await fetch('http://localhost:5001/api/worship-ministry/admin', {
          headers: {
        ...
        if (Array.isArray(data)) {
          ...
          setWorshipMinistrySections(validSections);
        } else {
  ...
  const updateSection = async (sectionId, updatedData) => {
    try {
      ...
      const response = await fetch(`http://localhost:5001/api/worship-ministry/${sectionId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`
        },
  ...
  const addNewSection = async () => {
    try {
      const newOrder = Math.max(...worshipMinistrySections.map(s => s.order), 0) + 1;
  ...
  if (authLoading) {
    return (
      <div className="edit-worship-ministry-page">
    ...
    return (
      <div className="edit-worship-ministry-page">
        <div className="error-container">
          <h2>Access Denied</h2>
          <p>You don't have permission to edit Worship Ministry content.</p>
          <button onClick={() => navigate('/worship-ministry')} className="btn-retry">
            Back to Worship Ministry
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="edit-worship-ministry-page">
      ...
      <div className="edit-header">
        <h1>Edit Worship Ministry</h1>
        <div className="header-actions">
          <button onClick={() => navigate('/worship-ministry')} className="btn-secondary">
            Back to Worship Ministry
          </button>
