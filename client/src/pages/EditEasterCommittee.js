...
const EditEasterCommittee = () => {
  ...
  // Fetch existing Easter Committee sections
  useEffect(() => {
    const fetchEasterCommitteeSections = async () => {
      try {
        setLoading(true);
        
        const response = await fetch('http://localhost:5001/api/easter-committee/admin', {
          headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to fetch Easter Committee sections: ${response.status} ${errorText}`);
        }
        ...
        setError(`Failed to load Easter Committee content: ${err.message}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  const updateSection = async (sectionId, updatedData) => {
    try {
      ...
      const response = await fetch(`http://localhost:5001/api/easter-committee/${sectionId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`
        },
  ...
  const deleteSection = async (sectionId) => {
    ...
    try {
      ...
      setEasterCommitteeSections(prev => prev.filter(section => section._id !== sectionId));
  ...
  const addNewSection = async () => {
    try {
      const newOrder = Math.max(...easterCommitteeSections.map(s => s.order), 0) + 1;
      ...
      setEasterCommitteeSections(prev => [...prev, newSection]);
  ...
  if (authLoading) {
    return (
      <div className="edit-easter-committee-page">
    ...
    return (
      <div className="edit-easter-committee-page">
        <div className="error-container">
          <h2>Access Denied</h2>
          <p>You don't have permission to edit Easter Committee content.</p>
          <button onClick={() => navigate('/easter-committee')} className="btn-retry">
            Back to Easter Committee
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="edit-easter-committee-page">
      ...
      <div className="edit-header">
        <h1>Edit Easter Committee</h1>
        <div className="header-actions">
          <button onClick={() => navigate('/easter-committee')} className="btn-secondary">
            Back to Easter Committee
          </button>
