...
const EditChildrenMinistry = () => {
  ...
  // Fetch existing Children Ministry sections
  useEffect(() => {
    const fetchChildrenMinistrySections = async () => {
      try {
        setLoading(true);
        
        const response = await fetch('http://localhost:5001/api/children-ministry/admin', {
          headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to fetch Children Ministry sections: ${response.status} ${errorText}`);
        }
        ...
        if (Array.isArray(data)) {
          ...
          setChildrenMinistrySections(validSections);
        } else {
          console.error('Expected array but got:', typeof data, data);
          setChildrenMinistrySections([]);
        }
        setError(null);
      } catch (err) {
        setError(`Failed to load Children Ministry content: ${err.message}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  const updateSection = async (sectionId, updatedData) => {
    try {
      ...
      const response = await fetch(`http://localhost:5001/api/children-ministry/${sectionId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`
        },
      ...
      setChildrenMinistrySections(prev => 
        prev.map(section => 
  ...
  const deleteSection = async (sectionId) => {
    ...
    try {
      ...
      setChildrenMinistrySections(prev => prev.filter(section => section._id !== sectionId));
  ...
  const addNewSection = async () => {
    try {
      const newOrder = Math.max(...childrenMinistrySections.map(s => s.order), 0) + 1;
      ...
      const response = await fetch('http://localhost:5001/api/children-ministry', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        },
      ...
      setChildrenMinistrySections(prev => [...prev, newSection]);
  ...
  if (authLoading) {
    return (
      <div className="edit-children-ministry-page">
    ...
    return (
      <div className="edit-children-ministry-page">
        <div className="error-container">
          <h2>Access Denied</h2>
          <p>You don't have permission to edit Children Ministry content.</p>
          <button onClick={() => navigate('/children-ministry')} className="btn-retry">
            Back to Children Ministry
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="edit-children-ministry-page">
      ...
      <div className="edit-header">
        <h1>Edit Children Ministry</h1>
        <div className="header-actions">
          <button onClick={() => navigate('/children-ministry')} className="btn-secondary">
            Back to Children Ministry
          </button>
      ...
      <div className="sections-container">
        {childrenMinistrySections && childrenMinistrySections.length > 0 ? (
          childrenMinistrySections
