...
const EditMensMinistry = () => {
  ...
  // Fetch existing Mens Ministry sections
  useEffect(() => {
    const fetchMensMinistrySections = async () => {
      try {
        setLoading(true);
        
        const response = await fetch('http://localhost:5001/api/mens-ministry/admin', {
          headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to fetch Mens Ministry sections: ${response.status} ${errorText}`);
        }
        ...
        setError(`Failed to load Mens Ministry content: ${err.message}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  const updateSection = async (sectionId, updatedData) => {
    try {
      ...
      const response = await fetch(`http://localhost:5001/api/mens-ministry/${sectionId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`
        },
  ...
  const deleteSection = async (sectionId) => {
    ...
    try {
      ...
      setMensMinistrySections(prev => prev.filter(section => section._id !== sectionId));
  ...
  const addNewSection = async () => {
    try {
      const newOrder = Math.max(...mensMinistrySections.map(s => s.order), 0) + 1;
      ...
      setMensMinistrySections(prev => [...prev, newSection]);
  ...
  if (authLoading) {
    return (
      <div className="edit-mens-ministry-page">
    ...
    return (
      <div className="edit-mens-ministry-page">
        <div className="error-container">
          <h2>Access Denied</h2>
          <p>You don't have permission to edit Mens Ministry content.</p>
          <button onClick={() => navigate('/mens-ministry')} className="btn-retry">
            Back to Mens Ministry
          </button>
        </div>
      </div>
    );
  ...
  if (loading) {
    return (
      <div className="edit-mens-ministry-page">
        <div className="loading-container">
          <div className="loading-spinner"></div>
          <p>Loading Mens Ministry sections...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="edit-mens-ministry-page">
        <div className="error-container">
          <h2>Error Loading Mens Ministry</h2>
          <p>{error}</p>
  ...
  return (
    <div className="edit-mens-ministry-page">
      ...
      <div className="edit-header">
        <h1>Edit Mens Ministry</h1>
        <div className="header-actions">
          <button onClick={() => navigate('/mens-ministry')} className="btn-secondary">
            Back to Mens Ministry
          </button>
