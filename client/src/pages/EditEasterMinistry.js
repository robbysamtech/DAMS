...
const EditEasterMinistry = () => {
    ...
    const fetchEasterSections = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to fetch Easter Ministry sections: ${response.status} ${errorText}`);
        }
        const data = await response.json();
        setEasterSections(data);
        setError(null);
      } catch (err) {
        setError(`Failed to load Easter Ministry content: ${err.message}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  const addNewSection = async () => {
    try {
      const newOrder = Math.max(...easterSections.map(s => s.order), 0) + 1;
    ...
    return (
      <div className="edit-easter-ministry-page">
        <div className="error-container">
          <h2>Access Denied</h2>
          <p>You don't have permission to edit Easter Ministry content.</p>
          <button onClick={() => navigate('/easter-ministry')} className="btn-retry">
