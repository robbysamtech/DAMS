...
const EditWomensMinistry = () => {
  ...
  // Fetch existing Womens Ministry sections
  useEffect(() => {
    const fetchWomensMinistrySections = async () => {
      try {
        setLoading(true);
        
        const response = await fetch('http://localhost:5001/api/womens-ministry/admin', {
          headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to fetch Womens Ministry sections: ${response.status} ${errorText}`);
        }
        ...
        if (Array.isArray(data)) {
          ...
          setWomensMinistrySections(validSections);
        } else {
          console.error('Expected array but got:', typeof data, data);
          setWomensMinistrySections([]);
        }
        setError(null);
      } catch (err) {
        setError(`Failed to load Womens Ministry content: ${err.message}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  const updateSection = async (sectionId, updatedData) => {
    try {
      ...
      const response = await fetch(`http://localhost:5001/api/womens-ministry/${sectionId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`
        },
      ...
      setWomensMinistrySections(prev => 
        prev.map(section => 
  ...
  const deleteSection = async (sectionId) => {
    ...
    try {
      ...
      setWomensMinistrySections(prev => prev.filter(section => section._id !== sectionId));
  ...
  const addNewSection = async () => {
    try {
      const newOrder = Math.max(...womensMinistrySections.map(s => s.order), 0) + 1;
      ...
      const response = await fetch('http://localhost:5001/api/womens-ministry', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        },
      ...
      setWomensMinistrySections(prev => [...prev, newSection]);
  ...
  if (authLoading) {
    return (
      <div className="edit-womens-ministry-page">
    ...
    return (
      <div className="edit-womens-ministry-page">
        <div className="error-container">
          <h2>Access Denied</h2>
          <p>You don't have permission to edit Womens Ministry content.</p>
          <button onClick={() => navigate('/womens-ministry')} className="btn-retry">
            Back to Womens Ministry
          </button>
        </div>
      </div>
    );
  ...
  if (loading) {
    return (
      <div className="edit-womens-ministry-page">
        <div className="loading-container">
          <div className="loading-spinner"></div>
          <p>Loading Womens Ministry sections...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="edit-womens-ministry-page">
        <div className="error-container">
          <h2>Error Loading Womens Ministry</h2>
          <p>{error}</p>
  ...
  return (
    <div className="edit-womens-ministry-page">
      ...
      <div className="edit-header">
        <h1>Edit Womens Ministry</h1>
        <div className="header-actions">
          <button onClick={() => navigate('/womens-ministry')} className="btn-secondary">
            Back to Womens Ministry
          </button>
      ...
      <div className="sections-container">
        {womensMinistrySections && womensMinistrySections.length > 0 ? (
          womensMinistrySections
