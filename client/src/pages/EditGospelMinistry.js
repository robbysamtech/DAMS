...
const EditGospelMinistry = () => {
    ...
    const fetchGospelMinistrySections = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          ...
          setGospelMinistrySections(validSections);
        } else {
          console.error('Expected array but got:', typeof data, data);
          setGospelMinistrySections([]);
        }
        setError(null);
      } catch (err) {
  ...
  const updateSection = async (sectionId, updatedData) => {
    try {
      ...
      setGospelMinistrySections(prev => 
        prev.map(section => 
  ...
  const deleteSection = async (sectionId) => {
    ...
    try {
      ...
      setGospelMinistrySections(prev => prev.filter(section => section._id !== sectionId));
  ...
  const addNewSection = async () => {
    try {
      ...
      setGospelMinistrySections(prev => [...prev, newSection]);
  ...
  return (
    <div className="edit-gospel-ministry-page">
      ...
      <div className="sections-container">
        {gospelMinistrySections && gospelMinistrySections.length > 0 ? (
