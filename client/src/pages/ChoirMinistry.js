...
const ChoirMinistry = () => {
  ...
  // Fetch Choir Ministry sections from MongoDB
  useEffect(() => {
    const fetchChoirMinistrySections = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error('Failed to fetch Choir Ministry sections');
        }
        const data = await response.json();
        setChoirMinistrySections(data);
        setError(null);
      } catch (err) {
        setError('Failed to load Choir Ministry content');
        console.error('Error fetching Choir Ministry sections:', err);
      } finally {
        setLoading(false);
      }
    };
