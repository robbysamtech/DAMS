...
const EditYouthMinistry = () => {
  ...
  const [youthMinistrySections, setYouthMinistrySections] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [alert, setAlert] = useState({ show: false, message: '', type: 'success' });

  // Fetch existing Youth Ministry sections
  useEffect(() => {
    const fetchYouthMinistrySections = async () => {
      try {
        setLoading(true);
        ...
        setError(`Failed to load Youth Ministry content: ${err.message}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  const updateSection = async (sectionId, updatedData) => {
    try {
      ...
      const response = await fetch(`http://localhost:5001/api/youth-ministry/${sectionId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`
        },
  ...
  const deleteSection = async (sectionId) => {
    ...
    try {
      ...
      setYouthMinistrySections(prev => prev.filter(section => section._id !== sectionId));
  ...
  const addNewSection = async () => {
    try {
      const newOrder = Math.max(...youthMinistrySections.map(s => s.order), 0) + 1;
      ...
      setYouthMinistrySections(prev => [...prev, newSection]);
  ...
  if (authLoading) {
    return (
      <div className="edit-youth-ministry-page">
    ...
    return (
      <div className="edit-youth-ministry-page">
        <div className="error-container">
          <h2>Access Denied</h2>
          <p>You don't have permission to edit Youth Ministry content.</p>
          <button onClick={() => navigate('/youth-ministry')} className="btn-retry">
  ...
  return (
    <div className="edit-youth-ministry-page">
      ...
      <div className="edit-header">
        <h1>Edit Youth Ministry</h1>
        <div className="header-actions">
          <button onClick={() => navigate('/youth-ministry')} className="btn-secondary">
            Back to Youth Ministry
          </button>
