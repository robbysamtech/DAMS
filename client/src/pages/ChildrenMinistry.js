...
const ChildrenMinistry = () => {
  ...
  // Fetch Children Ministry sections from MongoDB
  useEffect(() => {
    const fetchChildrenMinistrySections = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error('Failed to fetch Children Ministry sections');
        }
        const data = await response.json();
        setChildrenMinistrySections(data);
        setError(null);
      } catch (err) {
        setError('Failed to load Children Ministry content');
        console.error('Error fetching Children Ministry sections:', err);
      } finally {
        setLoading(false);
      }
    };
