...
const HarvestCommittee = () => {
  ...
  // Fetch Harvest Committee sections from MongoDB
  useEffect(() => {
    const fetchHarvestCommitteeSections = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error('Failed to fetch Harvest Committee sections');
        }
        const data = await response.json();
        setHarvestCommitteeSections(data);
        setError(null);
      } catch (err) {
        setError('Failed to load Harvest Committee content');
        console.error('Error fetching Harvest Committee sections:', err);
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="harvestcommittee-page">
      ...
      <section className="content-sections">
        {harvestcommitteeSections.length > 0 ? (
          harvestcommitteeSections.map((section, index) => (
            <div 
              key={section._id}
          ...
          <div className="no-sections-message">
            ...
            <p>Debug: harvestcommitteeSections.length = {harvestcommitteeSections.length}</p>
          </div>
        )}
      </section>
    </div>
