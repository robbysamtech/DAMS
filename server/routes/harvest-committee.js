...
const tileImageStorage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    cb(null, `harvest-committee-${uniqueSuffix}.${file.originalname.split('.').pop()}`);
  }
});
...
// @route   PATCH /api/harvest-committee/:id/tile-image
// @desc    Update tile image for a harvest-committee section
...
router.patch('/:id/tile-image', auth, (req, res, next) => {
  ...
  try {
    ...
    const section = await HarvestCommitteeSection.findById(req.params.id);
    if (!section) {
      return res.status(404).json({ error: 'Harvest Committee section not found' });
    }
    ...
    const processedImagePath = await processImage(req.file.path, uploadsDir, 'harvest-committee');
    const thumbnailPath = await generateThumbnail(processedImagePath, uploadsDir, 'harvest-committee');
...
// @route   PATCH /api/harvestcommittee/:id/tile-image
// @desc    Update tile image for a harvestcommittee section
// @access  Private (Admin/Editor)
router.patch('/:id/tile-image', auth, (req, res, next) => {
  console.log('PATCH tile-image middleware - before multer');
  console.log('Request headers:', req.headers);
  ...
  try {
    ...
    const section = await HarvestcommitteeMinistrySection.findById(req.params.id);
    if (!section) {
      return res.status(404).json({ error: 'harvestcommittee section not found' });
    }
    ...
    // Process the uploaded image
    const processedImagePath = await processImage(req.file.path, uploadsDir, 'harvestcommittee');
    const thumbnailPath = await generateThumbnail(processedImagePath, uploadsDir, 'harvestcommittee');
