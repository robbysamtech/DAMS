...
const EasterCommitteeSection = require('../models/EasterCommitteeSection');
...
const uploadsDir = path.join(__dirname, '../uploads/easterCommittee');
...
const tileImageStorage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    cb(null, `easter-committee-${uniqueSuffix}.${file.originalname.split('.').pop()}`);
  }
});
...
router.get('/', async (req, res) => {
  try {
    const sections = await EasterCommitteeSection.findActiveSections();
    res.json(sections);
  } catch (err) {
...
router.get('/admin', auth, async (req, res) => {
    ...
    const sections = await EasterCommitteeSection.findAllSections();
    res.json(sections);
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
...
router.post('/', auth, adminAuth, upload.fields([{ name: 'backgroundImage', maxCount: 1 }, { name: 'tileImage', maxCount: 1 }]), async (req, res) => {
  const { order, title, description } = req.body;try {
    const newSection = new EasterCommitteeSection({
      order, title, description, creator: req.user.id,
      backgroundImage: req.files && req.files['backgroundImage'] ? `/uploads/easterCommittee/${req.files['backgroundImage'][0].filename}` : undefined,
      tileImage: req.files && req.files['tileImage'] ? `/uploads/easterCommittee/${req.files['tileImage'][0].filename}` : undefined
    });
...
router.put('/:id', auth, upload.fields([{ name: 'backgroundImage', maxCount: 1 }, { name: 'tileImage', maxCount: 1 }]), async (req, res) => {
  const { order, title, description } = req.body;try {
    ...
    let section = await EasterCommitteeSection.findById(req.params.id);
    if (!section) return res.status(404).json({ msg: 'EasterCommittee section not found' });
    ...
    if (req.files && req.files['backgroundImage']) {
      ...
      section.backgroundImage = `/uploads/easterCommittee/${req.files['backgroundImage'][0].filename}`;
    }
    if (req.files && req.files['tileImage']) {
      ...
      section.tileImage = `/uploads/easterCommittee/${req.files['tileImage'][0].filename}`;
    }
...
router.delete('/:id', auth, async (req, res) => {
    ...
    const section = await EasterCommitteeSection.findById(req.params.id);
    if (!section) return res.status(404).json({ msg: 'EasterCommittee section not found' });
    ...
    await EasterCommitteeSection.deleteOne({ _id: req.params.id });
    res.json({ msg: 'EasterCommittee section removed' });
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
  }
...
// @route   PATCH /api/easter-committee/:id/tile-image
// @desc    Update tile image for a easter-committee section
...
router.patch('/:id/tile-image', auth, (req, res, next) => {
  ...
  try {
    ...
    const section = await EasterCommitteeSection.findById(req.params.id);
    if (!section) {
      return res.status(404).json({ error: 'Easter Committee section not found' });
    }
    ...
    const processedImagePath = await processImage(req.file.path, uploadsDir, 'easter-committee');
    const thumbnailPath = await generateThumbnail(processedImagePath, uploadsDir, 'easter-committee');
