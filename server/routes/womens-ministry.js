...
const tileImageStorage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    cb(null, `womens-ministry-${uniqueSuffix}.${file.originalname.split('.').pop()}`);
  }
});
...
// @route   PATCH /api/womens-ministry/:id/tile-image
// @desc    Update tile image for a womens-ministry section
...
router.patch('/:id/tile-image', auth, (req, res, next) => {
  ...
  try {
    ...
    const section = await WomensMinistrySection.findById(req.params.id);
    if (!section) {
      return res.status(404).json({ error: 'Womens Ministry section not found' });
    }
    ...
    const processedImagePath = await processImage(req.file.path, uploadsDir, 'womens-ministry');
    const thumbnailPath = await generateThumbnail(processedImagePath, uploadsDir, 'womens-ministry');
...
// @route   PATCH /api/womensministry/:id/tile-image
// @desc    Update tile image for a womensministry section
// @access  Private (Admin/Editor)
router.patch('/:id/tile-image', auth, (req, res, next) => {
  console.log('PATCH tile-image middleware - before multer');
  console.log('Request headers:', req.headers);
  ...
  try {
    ...
    const section = await WomensministryMinistrySection.findById(req.params.id);
    if (!section) {
      return res.status(404).json({ error: 'womensministry section not found' });
    }
    ...
    // Process the uploaded image
    const processedImagePath = await processImage(req.file.path, uploadsDir, 'womensministry');
    const thumbnailPath = await generateThumbnail(processedImagePath, uploadsDir, 'womensministry');
