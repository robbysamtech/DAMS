...
const MensMinistrySection = require('../models/MensMinistrySection');
...
const uploadsDir = path.join(__dirname, '../uploads/mensMinistry');
...
const tileImageStorage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    cb(null, `mens-ministry-${uniqueSuffix}.${file.originalname.split('.').pop()}`);
  }
});
...
router.get('/', async (req, res) => {
  try {
    const sections = await MensMinistrySection.findActiveSections();
    res.json(sections);
  } catch (err) {
...
router.post('/', auth, adminAuth, upload.fields([{ name: 'backgroundImage', maxCount: 1 }, { name: 'tileImage', maxCount: 1 }]), async (req, res) => {
  const { order, title, description } = req.body;try {
    const newSection = new MensMinistrySection({
      order, title, description, creator: req.user.id,
      backgroundImage: req.files && req.files['backgroundImage'] ? `/uploads/mensMinistry/${req.files['backgroundImage'][0].filename}` : undefined,
      tileImage: req.files && req.files['tileImage'] ? `/uploads/mensMinistry/${req.files['tileImage'][0].filename}` : undefined
    });
...
router.put('/:id', auth, upload.fields([{ name: 'backgroundImage', maxCount: 1 }, { name: 'tileImage', maxCount: 1 }]), async (req, res) => {
  const { order, title, description } = req.body;try {
    ...
    let section = await MensMinistrySection.findById(req.params.id);
    ...
    if (req.files && req.files['backgroundImage']) {
      ...
      section.backgroundImage = `/uploads/mensMinistry/${req.files['backgroundImage'][0].filename}`;
    }
    if (req.files && req.files['tileImage']) {
      ...
      section.tileImage = `/uploads/mensMinistry/${req.files['tileImage'][0].filename}`;
    }
...
router.delete('/:id', auth, async (req, res) => {
    ...
    await MensMinistrySection.deleteOne({ _id: req.params.id });
    res.json({ msg: 'MensMinistry section removed' });
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
  }
...
// @route   PATCH /api/mens-ministry/:id/tile-image
// @desc    Update tile image for a mens-ministry section
...
router.patch('/:id/tile-image', auth, (req, res, next) => {
  ...
  try {
    ...
    const section = await MensMinistrySection.findById(req.params.id);
    if (!section) {
      return res.status(404).json({ error: 'Mens Ministry section not found' });
    }
    ...
    const processedImagePath = await processImage(req.file.path, uploadsDir, 'mens-ministry');
    const thumbnailPath = await generateThumbnail(processedImagePath, uploadsDir, 'mens-ministry');
...
// @route   PATCH /api/mensministry/:id/tile-image
// @desc    Update tile image for a mensministry section
// @access  Private (Admin/Editor)
router.patch('/:id/tile-image', auth, (req, res, next) => {
  console.log('PATCH tile-image middleware - before multer');
  console.log('Request headers:', req.headers);
  ...
  try {
    ...
    const section = await MensministryMinistrySection.findById(req.params.id);
    if (!section) {
      return res.status(404).json({ error: 'mensministry section not found' });
    }
    ...
    // Process the uploaded image
    const processedImagePath = await processImage(req.file.path, uploadsDir, 'mensministry');
    const thumbnailPath = await generateThumbnail(processedImagePath, uploadsDir, 'mensministry');
