...
const ChoirMinistrySection = require('../models/ChoirMinistrySection');
...
const uploadsDir = path.join(__dirname, '../uploads/choirMinistry');
...
router.get('/admin', auth, async (req, res) => {
    ...
    const sections = await ChoirMinistrySection.findAllSections();
    res.json(sections);
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
...
router.put('/:id', auth, upload.fields([{ name: 'backgroundImage', maxCount: 1 }, { name: 'tileImage', maxCount: 1 }]), async (req, res) => {
  ...
  try {
    ...
    let section = await ChoirMinistrySection.findById(req.params.id);
    if (!section) return res.status(404).json({ msg: 'ChoirMinistry section not found' });
    ...
    if (req.files && req.files['backgroundImage']) {
      ...
      section.backgroundImage = `/uploads/choirMinistry/${req.files['backgroundImage'][0].filename}`;
    }
    if (req.files && req.files['tileImage']) {
      ...
      section.tileImage = `/uploads/choirMinistry/${req.files['tileImage'][0].filename}`;
    }
...
router.delete('/:id', auth, async (req, res) => {
    ...
    await ChoirMinistrySection.deleteOne({ _id: req.params.id });
    res.json({ msg: 'ChoirMinistry section removed' });
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
  }
