...
const tileImageStorage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    cb(null, `gospel-ministry-${uniqueSuffix}.${file.originalname.split('.').pop()}`);
  }
});
...
// @route   PATCH /api/gospel-ministry/:id/tile-image
// @desc    Update tile image for a gospel-ministry section
...
router.patch('/:id/tile-image', auth, (req, res, next) => {
  ...
  try {
    ...
    const section = await GospelMinistrySection.findById(req.params.id);
    if (!section) {
      return res.status(404).json({ error: 'Gospel Ministry section not found' });
    }
    ...
    const processedImagePath = await processImage(req.file.path, uploadsDir, 'gospel-ministry');
    const thumbnailPath = await generateThumbnail(processedImagePath, uploadsDir, 'gospel-ministry');
